...
get_data <- function(index, n, dependence = 0){
  ...
  if(var(y) == 0){
    k <- 1
  } else {k <- as.numeric(sqrt(var(y)/(4*var(noise))))}
  y_new <- y+k*rnorm(n=n)
...
alpha <- 0.2
run_sim <- function(alpha = 0.5, stop_par = 2500, nu = 0.1, n_group_empty = 5,
                    n_group_full = 5, n_group_half = 5, n_vars_empty = 15,
                    n_vars_full = 15, n_vars_half = 15, n = 50, dependence = 0, 
                    nlam = 10, k_folds = 3, iteration, scenario) {
  ...
  for(i in 1:10){
    ...
    if(is.character(sgl_model)){
      ...
      is_error_sgl <- FALSE}
    results <- results %>% 
      rbind(data.frame(model = 'sgl', Alpha = alpha,lambda = lambda,
...
results_df <- params %>%
  ...
  unnest(cols = res)
tictoc::toc()
saveRDS(file = 'simulation_results/simulation_02.RDS', results_df)
plan(sequential)

# Analysis of results
results_df <- readRDS('simulation_results/simulation_02.RDS')
...
res_df <- results_df %>%
  ...
  filter((model == 'sgl' & lambda == lambda_opt) | (model != 'sgl' & rmse == min(rmse))) 

# Figure 2 from supplement 
plot_rmse <- res_df %>%
  group_by(scenario) %>%
  mutate(rmse = as.numeric(scale(log(rmse+0.00001)))) %>%
  ungroup() %>%
  group_by(alpha,n_group_empty,n_group_full, n_group_half, n_vars_empty,
...
# Simulation Figure 5 for manuscript
plot_sgb <- results_df %>%
  filter(model == 'sgb df') %>%
...
ggsave(plot = big_plot, filename = 'simulation_figures_tables/sim_results.png', dpi = 900, width = 14, height = 10)
...
res_df %>% 
  ...
  writexl::write_xlsx('simulation_figures_tables/sim_results_min.xlsx')
